...
public class Competition_297 {
    ...
    public int distributeCookies(int[] cookies, int k) {
        ...
        processDistribute(cookies, childCookies, cookies.length-1);
        return childCookies[k];
    }

    public void processDistribute(int[] cookies, int[] childCookies, int cookiesIndex){
        if(cookiesIndex < 0){
            int maxValue = Integer.MIN_VALUE;
            for (int i = 0; i < childCookies.length-1; i++) {
                maxValue = Math.max(childCookies[i], maxValue);
            }
            childCookies[childCookies.length-1] = Math.min(childCookies[childCookies.length-1], maxValue);
        }

        int childrenWithoutCookies = 0;
        for (int i = 0; i < childCookies.length-1; i++) {
            if(childCookies[i] >= childCookies[childCookies.length-1]) return;
            if(childCookies[i] == 0) childrenWithoutCookies++;
        }

        if(childrenWithoutCookies > cookiesIndex + 1) return;

        for (int i = 0; i < childCookies.length-1; i++) {
            if(cookiesIndex == cookies.length - 1 && i > 0) return;

            childCookies[i] += cookies[cookiesIndex];
            processDistribute(cookies, childCookies, cookiesIndex-1);
            childCookies[i] -= cookies[cookiesIndex];
        }
    }
...
//        System.out.println(obj.minPathCost2(grid, moveCost));
        ...
        System.out.println(obj.distributeCookies(cookies, 3));
    }
}
